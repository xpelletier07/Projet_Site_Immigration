...
import { updateClientProfile, uploadDocument } from "../services/client.service.jsx";
...
const ACTIVITY = [
	{ date: "AUJOURD'HUI", title: "Dossier en révision", desc: "Votre dossier est en cours de révision finale par un agent.", icon: "📋" },
	{ date: "05 FÉVRIER 2024", title: "Examen médical validé", desc: "Les résultats ont été reçus et confirmés conformes.", icon: "✓" },
	{ date: "15 JANVIER 2024", title: "Accusé de réception", desc: "Confirmation officielle de la réception de votre dossier complet.", icon: "✉" },
];

// ─── Page principale ──────────────────────────────────────────────────────────
export default function MyCasePage({ bundle, onSelectDossier, onRefresh }) {
	const toast = useToast();
	const fileInputRef = useRef();
	const [showEdit, setShowEdit] = useState(false);
	const [uploading, setUploading] = useState(false);
	// Copie locale du client pour les updates de profil sans re-fetch
	const [localClient, setLocalClient] = useState(null);

	const { hasDossier, client: bundleClient, dossier, dossiers = [], documents = [] } = bundle || {};
	const client = localClient ?? bundleClient;
	...
	function handleProfileSaved(updates) {
		setLocalClient((prev) => ({ ...(prev ?? bundleClient), ...updates }));
		setShowEdit(false);
	}
						...
						<div className="flex justify-between items-center" style={{ gap: "16px", flexWrap: "wrap" }}>
							<div>
								...
								<p style={{ color: "var(--muted)", marginTop: "6px", fontSize: "0.9rem" }}>
									Consultez l'état d'avancement de votre demande #{dossier.id_dossier}, gérez vos documents officiels et mettez à jour vos informations personnelles.
								</p>
							</div>
											...
											<div className="doc-item-left">
												...
												<div>
													...
													<div className="doc-date">
														Soumis le {new Date().toLocaleDateString("fr-CA")}
													</div>
												</div>
											</div>
											<div className="flex gap-2 items-center">
						...
						<div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
							...
							<div style={{ background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: "14px", padding: "20px" }}>
								...
								<p style={{ fontSize: "0.82rem", color: "#92400e", lineHeight: 1.6 }}>
									Si vous constatez une erreur dans vos documents, veuillez contacter le centre de soutien légal avant toute modification.
								</p>
							</div>
						</div>
					</div>
