...
export function GestionsClients() {
  ...
  async function handleDelete(id_dossier) {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer ce dossier ?")) {
      await apiFetch(`/dossiers/${id_dossier}`, {
        method: "DELETE",
      }).then(() => {
        setClients(clients.filter((c) => c.dossier.id_dossier !== id_dossier));
      })
    }
  }
  ...
  useEffect(() => {
    async function fetchClients() {
      try {
        ...
        const contenufiltré = contenu.filter((c) => {
          const matchNom = c.dossier.nom.toLowerCase().includes(search.toLowerCase());
          const matchDossier = c.dossier.id_dossier.toString().includes(search.toLowerCase());
          const matchStatus = statusFilter === "Tous les statuts" || c?.details?.factures[0]?.statut === statusFilter || false;
          const matchType = typeFilter === "Tous les types" || c?.details?.typeDemandes[0]?.Type_Demande === typeFilter || false;

          return (matchNom || matchDossier) && matchStatus && matchType;
        })
    ...
    fetchClients();
  }, [search, statusFilter, typeFilter, isClick]);
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        <main style={{ flexGrow: 1, maxWidth: 1400, margin: "0 auto", width: "100%", padding: "3rem 1.5rem" }}>
              ...
              return (
                <div key={client.id} className="ledger-row">
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                    ...
                    >
                      {client.dossier.nom.charAt(0) + client.dossier.prenom.charAt(0)}
                    </div>
                    <div>
                      <div className="client-name">{client.dossier.prenom}</div>
                      <div className="client-email">{/*client.email*/}</div>
                    </div>
                  </div>

                  {/* Dossier */}
                  <div className="dossier-num">#{client.dossier.id_dossier}</div>
                  ...
                  <div className="col-actions">
                    <button
                      ...
                      title="Modifier"
                      onClick={() => {
                        setSelectedClientId(client.dossier.id_client);
                        setShowModal(true);
                      }}
                    >
                      ✏️
          ...
          <div className="pagination-area">
            <span className="pagination-info">Affichage de 10 sur {clients.length} clients</span>
            <nav className="pagination" role="navigation" aria-label="pagination" style={{ display: "flex", gap: "0.5rem" }}>
              <ul className="pagination-list">
                <button className="pag-btn arrow" disabled={isClick === 0}
                  onClick={() => { setIsClick(isClick - 1); }}>‹</button>

                {
                  numerotation().map(num =>
                    <li key={num.index}>
                      <button onClick={() => { setIsClick(num.index) }} className={num.clic}>
                        {num.index + 1}
                      </button>
                    </li>
                  )
                }
                <button className="pag-btn arrow" disabled={isClick === nbpages - 1}
                  onClick={() => { setIsClick(isClick + 1); }}>›</button>
              </ul>
            </nav>
          </div>
