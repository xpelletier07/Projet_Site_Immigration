...
export async function getAllDossiersActifs(token) {
	...
	const dossiersAvecDetails = await Promise.all(
		data.map(async (d) => {
			const dossier = await apiFetch(`/dossiers/${d.id_dossier}`, {
				method: "GET",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${token}`,
...
export async function getClientDossier(token) {
	...
	const resultats = await Promise.all(
		data.map(async (e) => {
			const dossier = await apiFetch(`/dossiers/client/${e.id_client}`);
	...
	const clientsSansDossier = resultats.filter(
		(r) => !r.dossier || r.dossier.length === 0,
	);
	...
	const contenu = {
		a: clientsSansDossier.length,
		b: clientsAvecDossier.length,
		c: resultats.length,
	};
	return contenu;
}
